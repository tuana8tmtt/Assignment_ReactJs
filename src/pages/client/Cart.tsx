                ...
                <div className="flex shadow-md my-10">
                    <div className="w-3/4 bg-white px-10 py-10">
                        ...
                        <div className="flex mt-10 mb-5">
                            ...
                            <h3 className="font-semibold text-center text-gray-600 text-xs uppercase w-1/5 text-center">Tổng cộng</h3>
                        </div>
                        {cart?.map((item: IProduct) => (
                            <div className="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5">
                                ...
                                <span className="text-center w-1/5 font-semibold text-sm">{Money(item.price)}</span>
                                <span className="text-center w-1/5 font-semibold text-sm">{Money(item.price * item.quantity)}</span>
                            </div>
                        ))}
                    ...
                    <div id="summary" className="w-1/4 px-8 py-10">
                        <h1 className="font-semibold text-2xl border-b pb-8">Sản phẩm đã chọn</h1>
                        {cart?.map((item: any) => (
                            <div className="flex justify-between mt-10 mb-5">
                                <span className="font-semibold text-sm uppercase">{item.name}</span>
                                <span className="font-semibold text-sm">{Money(item.price * item.quantity)}</span>
                            </div>
                        ))}
